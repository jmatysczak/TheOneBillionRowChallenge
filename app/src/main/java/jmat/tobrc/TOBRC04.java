...
public class TOBRC04 extends AbstractTOBRC {
			...
			while(in.available() > 0) {
				...
				for(var i = offset; i < bytesEnd; i++) {
					if(calculatingHash) {
						...
						if(bytes[i] == '\n') {
							measurementSummaries.addOrUpdate(stationHash, bytes, stationStart, stationFinish, measurement * sign);
	...
	class HashMap {
		...
		private MeasurementSummary[] measurementSummaries = new MeasurementSummary[1024];
		private int numberOfMeasurementSummaries = 0;
		private double measurementSummariesTheshold = measurementSummaries.length * LOAD_FACTOR;

		public void addOrUpdate(int hashCode, final byte[] bytes, final int start, final int finish, final int measurement) {
			...
			for(var j = startIndex; true; j = ((j + 1) % measurementSummaries.length)) {
				...
				if(measurementSummary == null) {
					var stationAsBytes = Arrays.copyOfRange(bytes, start, finish);
					var stationAsString = new String(stationAsBytes, StandardCharsets.UTF_8);
					measurementSummaries[j] = new MeasurementSummary(hashCode, stationAsBytes, stationAsString, measurement);
					...
					if(numberOfMeasurementSummaries > measurementSummariesTheshold) {
						...
						measurementSummariesTheshold = measurementSummaries.length * LOAD_FACTOR;
					}

					break;
				} else {
					var currentLength = finish - start;
