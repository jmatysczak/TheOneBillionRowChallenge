...
public class TOBRC05 extends AbstractTOBRC {
	public static void main(final String args[]) throws Exception {
		new TOBRC05().run(args);
	}
						...
						if(bytes[i] == '\n') {
							...
							for(var j = hashCode % measurementSummaries.length; true; j = ((j + 1) % measurementSummaries.length)) {
								...
								if(measurementSummary == null) {
									...
									if(currentLength == otherStationAsBytes.length) {
										var k = currentLength;
										while(--k >= 0 && bytes[stationStart + k] == otherStationAsBytes[k]);
										if(k == -1) {
